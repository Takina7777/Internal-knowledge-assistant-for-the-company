...
class UserStore:
    ...
    def _seed_admin(self) -> None:
        ...
        if self.get_by_username(s.ADMIN_USERNAME) is None:
            self.create_user(s.ADMIN_USERNAME, s.ADMIN_PASSWORD, display_name="管理员", role="admin")

    # ---- 用户 ----

    def create_user(self, username: str, password: str, display_name: str = "", role: str = "user") -> dict:
        with self._connect() as conn:
            conn.execute(
                "INSERT INTO users (username, password_hash, display_name, role, is_active, created_at)"
                " VALUES (?,?,?,?,1,?)",
                (username, hash_password(password), display_name, role, datetime.now().isoformat()),
            )
    ...
    def get_by_username(self, username: str) -> dict | None:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT id, username, display_name, role, is_active FROM users WHERE username=?", (username,)
            ).fetchone()
    ...
    def get_by_id(self, user_id: int) -> dict | None:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT id, username, display_name, role, is_active FROM users WHERE id=?", (user_id,)
            ).fetchone()
        return self._row_to_user(row)
    ...
    def authenticate(self, username: str, password: str) -> dict | None:
        ...
        if not row or row[5] != 1 or not verify_password(password, row[2]):
            return None
        return {"id": row[0], "username": row[1], "display_name": row[3], "role": row[4]}
    ...
    def _row_to_user(row) -> dict | None:  # noqa: ANN001
        """users 表 5 列查询：id, username, display_name, role, is_active。"""
        if not row:
            return None
        return {
            "id": row[0],
